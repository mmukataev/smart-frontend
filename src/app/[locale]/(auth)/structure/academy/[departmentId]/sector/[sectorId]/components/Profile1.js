...
export default function Profile({ id }) {
  ...
  useEffect(() => {
    const fetchEmployee = async () => {
      try {
        const res = await fetch(`https://devapi-smart.apa.kz/employee/${id}/`);
  ...
  if (!employeeData) return <p className="text-center text-red-500">Сотрудник не найден</p>;

  const { employee, department, sector, allEmployees, chief } = employeeData;
  ...
  return (
    <div className="space-y-4 w-fit mx-auto">
      <div className="flex space-x-4">
        ...
        <div className="flex flex-col justify-between">
          ...
          <p className='leading-[18px]'>
            <b className='text-[var(--customDark)] text-[18px]'>
      ...
      <div className="flex gap-[5px]">
        <img src="/icons/systems/case.svg" className="w-[24px] h-[24px] inline-block mr-2" />
        <div>
          ...
          <p className="text-[14px] text-[#15312A] font-bold mb-[5px]">{getWorkDuration(employee.work_start_date)}</p>
        </div>
      </div>

      <div className="flex gap-[5px]">
        <img src="/icons/systems/phone.svg" className="w-[24px] h-[24px] inline-block mr-2" />
        <p className="text-[14px] text-[#267962] font-bold">Телефон:</p>
        <p className="text-[14px] text-[#15312A] font-bold mb-[5px]">{employee.user_phone || 'Не указан'}</p>
      </div>

      <div className="flex gap-[5px]">
        <img src="/icons/systems/mail.svg" className="w-[24px] h-[24px] inline-block mr-2" />
        <p className="text-[14px] text-[#267962] font-bold">Email:</p>
        <p className="text-[14px] text-[#15312A] font-bold mb-[5px]">{employee.user_email || 'Не указан'}</p>
      </div>
        ...
        <div className="flex gap-[5px] items-center">
          ...
          <div>
            ...
            <p className="text-[14px] text-[#15312A] font-bold">
              {departmentHead.user_surename} {departmentHead.user_name} {departmentHead.user_patronymic}
            </p>
          </div>
        </div>
      )}
      ...
      {sectorHead && !employee.is_sector_head && (
        <div className="flex gap-[5px] items-center">
          ...
          <div>
            ...
            <p className="text-[14px] text-[#15312A] font-bold">
              {sectorHead.user_surename} {sectorHead.user_name} {sectorHead.user_patronymic}
            </p>
          </div>
        </div>
      )}
