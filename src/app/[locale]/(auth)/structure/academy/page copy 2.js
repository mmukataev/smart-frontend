...
async function getSubordinations() {
 ...
 //const res = await fetch('https://api-smart.apa.kz:8050/subordinations/');
...
async function getSectors(departmentId) {
  ...
  employees.forEach(employee => {
    ...
    if (!sectorId) return; // Пропустить, если sector или id нет
...
export default function StructurePage() {
 ...
 const [chiefs_sector, setChiefsSector] = useState({});
  ...
  const handleDepartmentClick = async (departmentId) => {
    ...
    if (!sectors[departmentId]) {
      ...
      const allEmployees = sectorData.flatMap(sector => sector.employees || []);
        console.log("sectorData:", allEmployees);

//         allEmployees.forEach(emp => {
//   console.log(`emp: ${emp.user_surename} | chief: ${emp.is_chief} | departmentId: ${emp.department?.id}`);
// });
    // Фильтрация руководителей секторов
      const chiefSectorList = allEmployees.filter(
        emp =>
          emp?.is_sector_head === true &&
          emp?.sector?.id &&
          emp?.department?.id &&
          Number(emp.department.id) === Number(departmentId)
      );

      // Формируем объект руководителей секторов: { [sectorId]: chief }
      const chiefsSectorObject = {};
      chiefSectorList.forEach(emp => {
        if (emp?.sector?.id) {
          chiefsSectorObject[emp.sector.id] = emp;
        }
      });

      setChiefsSector(prev => ({ ...prev, ...chiefsSectorObject }));

      if (chiefSectorList.length > 0) {
        console.log("✅ Найдены sector head:", chiefSectorList);
      } else {
        console.log("❌ Руководители секторов не найдены для отдела:", departmentId);
      }

    const chief = allEmployees.find(emp =>
      emp.is_chief === true && Number(emp.department?.id) === Number(departmentId)
    );

     if (chief) {  
      setChiefs(prev => ({ ...prev, [departmentId]: chief }));
        console.log("✅ Chief найден:", chief);
      } else {
        console.log("❌ Chief не найден для отдела:", departmentId);
      }
            ...
            <div className="mb-[10px]">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1"> 
                  ...
                  <p className="font-bold text-[12px] text-[#267962] mt-1 leading-[12px]">{person[locale]}</p>
                </div>
              </div>
            </div>
                  ...
                  <li key={dept.id}>
                    ...
                    >
                      {dept[locale]}
                              ...
                              {/* Показываем руководителя */}
                          {chiefs[dept.id] && (
                            <>
                            <Link
                                href={`/${locale}/structure/academy/${dept.id}/sector/${sectors[dept.id][0].id}`}
                                className="text-[#267962] text-[12px] pl-2 py-2 cursor-pointer hover:text-[#1b5c4f] transition block"
                                title="Перейти к первому сектору"
                              >
                                <small className='text-[12px]'>Руководитель:</small>
                                <br/>
                                <b >{chiefs[dept.id].user_surename} {chiefs[dept.id].user_name} {chiefs[dept.id].user_patronymic}</b>
                              </Link>
                      ...
                      <div className="ml-4 mt-2 space-y-2">
                        ...
                        {loading[dept.id] ? (
                          ...
                          sectors[dept.id]?.map(sector => (
                            ...
                            >
                              {chiefs_sector[sector.id]?.user_surname} {chiefs_sector[sector.id]?.user_name} {chiefs_sector[sector.id]?.user_patronymic} - {chiefs_sector[sector.id]?.position?.[locale]} {sector[locale]}
                            </Link>
                          ))
                        )}
                      </div>
